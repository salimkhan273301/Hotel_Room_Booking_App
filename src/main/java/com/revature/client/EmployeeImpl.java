...
public class EmployeeImpl extends Employee {
	...
	void request() throws Exception {
		...
		int ch;
		while(true) {
			System.out.println("what do you want, Please select the option?");
			System.out.println("Press 1 to Request a Cab");
			System.out.println("Press 2 to See booking history");
			System.out.println("Press 3 to see last boooking status");
			...
			case 1:
			{
				...
				pst = con.prepareStatement("insert into request (eid,sid) values(?,?)");
			...
			case 2:
			{
				...
				pst = con.prepareStatement("select * from request where eid=?");
				pst.setInt(1,this.getId());
				rs = pst.executeQuery();
				while(rs.next()) {
			...
			case 3:
			{
				...
				pst = con.prepareStatement("select sid,booking_id from request where eid=? order by request_id desc limit 1");
				pst.setInt(1,this.getId());
				rs = pst.executeQuery();
				if(rs.next()) {
					int sta = rs.getInt(1);
