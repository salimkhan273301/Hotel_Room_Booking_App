...
private static final Logger logger = Logger.getLogger("ManagerImpl.class");
	...
	void accept() throws Exception{
		...
		PreparedStatement pt = con.prepareStatement("select * from request where sid=1");
		...
		while(rs.next()) {
			...
			if(m=='y') {
				PreparedStatement pst = con.prepareStatement("update request set sid=2 where request_id=?");
				...
				PreparedStatement pst = con.prepareStatement("update request set sid=3 where request_id=?");
				pst.setInt(1, rs.getInt(1));
				int a = pst.executeUpdate(); //update
				System.out.println(a+" Row Affected! \nRequest Rejected!");
